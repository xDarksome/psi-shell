...
// https://github.com/elkowar/eww/blob/master/crates/eww/src/widgets/circular_progressbar.rs
...
impl ObjectSubclass for CircProgPriv {
    ...
    fn class_init(klass: &mut Self::Class) { klass.set_css_name("circular-progress"); }
}
...
impl ContainerImpl for CircProgPriv {
    fn add(&self, container: &Self::Type, widget: &gtk::Widget) {
        if let Some(content) = &*self.content.borrow() {
            ...
            eprintln!("Error, trying to add multiple children to a circular-progress widget");
    ...
    fn draw(&self, widget: &Self::Type, cr: &cairo::Context) -> Inhibit {
        let res: Result<(), String> = (|| {
            ...
            let bg_color: gdk::RGBA = styles
                ...
                .get()
                .map_err(|e| format!("get background-color property: {e}"))?;
            ...
            cr.save().map_err(|e| format!("cairo save: {e}"))?;
            ...
            cr.fill().map_err(|e| format!("cairo fill: {e}"))?;
            ...
            cr.set_fill_rule(cairo::FillRule::EvenOdd); // Substract one circle from the other
            cr.fill().map_err(|e| format!("cairo fill: {e}"))?;
            cr.restore().map_err(|e| format!("cairo restore: {e}"))?;
        ...
        if let Err(e) = res {
            log::error!("Failed to draw CircProg: {e}");
        };
...
fn perc_to_rad(n: f64) -> f64 { (n / 100f64) * 2f64 * std::f64::consts::PI }
